# Change regular expression reading in document id (AN) to insure uniqueness
#   add + to end of line 51 (also, comment out page number extraction, line 45-46,112)
readFactivaHTML3 <- FunctionGenerator(function(elem, language, id) {
...
#    pg <- if(is.na(data[["PG"]][[1]])) NA # mpc: was getting error on this line, so removed it
#    else as.integer(regmatches(data[["PG"]], regexpr("^[[:digit:]]+", data[["PG"]])))[[1]]
    ...
    m <- regmatches(data[["AN"]], regexec("^([A-Za-z]+)0*[1-9][0-9]([0-9][0-9][0-3][0-9][0-3][0-9])([A-Za-z0-9]+)", # mpc: added + at end
                                          data[["AN"]]))[[1]]
